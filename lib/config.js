...
export const normalizeSortLocal = (sortString) => {
  // regexp to validate the query param
  const validPattern = /^(name|name-1|price|price-1)$/
  if (!validPattern.test(sortString)) throw createHttpError(400, '400 BAD REQUEST | ERROR in URL: SORT query param should match <name|name-1|price|price-1> pattern')
  // return normalized sort
  if (sortString === 'name') return 'Sort by name from A to Z'
  if (sortString === 'name-1') return 'Sort by name from Z to A'
  if (sortString === 'price') return 'Sort by price ascendent'
  if (sortString === 'price-1') return 'Sort by price descendent'
}

export const normalizeTag = (tagString) => {
  const validPattern = /^(motor|mobile|lifestyle|work)$/i
  if (!validPattern.test(tagString)) throw createHttpError(400, '400 BAD REQUEST | ERROR in URL: SORT query param should match <motor|lifestyle|mobile|work> pattern')
