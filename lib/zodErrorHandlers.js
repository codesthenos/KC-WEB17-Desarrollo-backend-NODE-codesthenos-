...
export const handleProductValidationError = (error, res, productName, productPrice, productImage, productTags) => {
  const errorMessage = error.errors.map(err =>
    `
      message: ${err.message}
    `).join('')
  setLocals(res, { productName, productPrice, productImage, productTags, title: CREATE_PRODUCT_TITLE, error: errorMessage })
